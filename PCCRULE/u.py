...
def extract_info(data):
    ...
    patterns = [
        r"(user-plane predefined-rules predefined-rule[\s\S]*?!(?=\n[^ ])\n)",
        r"(user-plane packet-detection predefined-pdrs predefined-pdr[\s\S]*?!(?=\n[^ ])\n)",
        r"(user-plane packet-detection applications application[\s\S]*?!(?=\n[^ ])\n)",
        r"(user-plane packet-detection filters filter[\s\S]*?!(?=\n[^ ])\n)"  # New pattern added
    ]
